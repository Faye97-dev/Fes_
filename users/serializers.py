...
class Employe_UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Employee
        fields = ('username', 'first_name', 'last_name',
                  'role', 'password', 'tel', 'email', 'adresse', 'agence')
        extra_kwargs = {'password': {'write_only': True}}
...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        if self.user.role == MyUser.RESPONSABLE_AGENCE:
            ...
            data['user'] = model_to_dict(responsable, fields=['id', 'username', 'first_name', 'last_name',
            ...
            data['agence'] = model_to_dict(agence)
            data['user'] = model_to_dict(employe, fields=['id', 'username', 'first_name', 'last_name',
                                                          'role', 'tel', 'email', 'adresse', 'start_date', 'is_active', 'last_login'])
        elif self.user.role == MyUser.AGENT_COMPENSATION:
            agent = Agent.objects.get(id=self.user.id)
            data['user'] = model_to_dict(agent, fields=['id', 'username', 'first_name', 'last_name',
                                                        'role', 'tel', 'email', 'adresse', 'start_date', 'is_active', 'last_login'])
